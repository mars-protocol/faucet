...
const Home = () => {
  ...
  const handleSubmit = () => {
    const faucetUrl = 'https://testnet-faucet.marsprotocol.io/claim'
    setSending(true)
    ...
    axios
      ...
      .then((res) => {
        ...
        if (response.code) {
          setErrorText(`Error: ${response.raw_log || `code: ${response.code}`}`)
          setSending(false)
          setSuccess(false)
          setError(true)
        } else {
          setTxUrl(`https://testnet-explorer.marsprotocol.io/transactions/${response.txhash}`)
          setSending(false)
          setSuccess(true)
          setError(false)
        }
    ...
    <section className={styles.home}>
      ...
      {success ? (
        <div className={styles.content}>
          <h2 className={styles.subtitle}>{t('common.transactionSuccessful')}</h2>
          <div className={styles.status}>
            <SuccessSVG />
          </div>
          <h3 className={styles.statusText}>{t('faucet.tokensClaimed')}</h3>
          <dl className={styles.data}>
            <dt className={styles.label}>{t('common.received')}</dt>
            <dd className={styles.value}>
              {formatValue(sentAmount / 1000000, 2, 2, true, false, ' MARS', false, false)}
            </dd>
          </dl>
          ...
          <div className={styles.button}>
            <Button disabled={sending} text={t('common.close')} onClick={resetUI} />
          </div>
        </div>
      ) : error ? (
        <div className={styles.content}>
          <h2 className={styles.subtitle}>{t('common.transactionFailed')}</h2>
          <div className={styles.status}>
            <FailedSVG />
          </div>
        ...
        <div className={styles.content}>
          ...
          <div className={styles.button}>
            <Button
              ...
              text={sending ? <CircularProgress /> : t('faucet.sendMeTokens')}
